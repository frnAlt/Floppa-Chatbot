...
module.exports = {
  ...
  onStart: async function ({ api, event, message, args }) {
    ...
    try {
      const apiUrl = `https://toshiro-api-editz6t9.vercel.app/api/canvas/patrick?avatar=${encodeURIComponent(imageUrl)}`;
      const stream = await global.utils.getStreamFromURL(apiUrl, "patrick.png");

      return message.reply({
        body: "⭐ *Patrick Star Canvas*",
        attachment: stream
      });
    } catch (error) {
      console.error("Patrick canvas error:", error.message);
