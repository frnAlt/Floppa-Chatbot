...
module.exports = {
  ...
  onStart: async function ({ api, event, message, args }) {
    ...
    try {
      const apiUrl = `https://toshiro-api-editz6t9.vercel.app/api/canvas/rip?image=${encodeURIComponent(imageUrl)}`;
      const stream = await global.utils.getStreamFromURL(apiUrl, "rip.png");
      ...
      console.error("RIP error:", error.message);
      return message.reply(`❌ Failed to generate RIP effect: ${error.message}`);
    }
  }
};