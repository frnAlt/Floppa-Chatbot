...
module.exports = {
  config: {
    name: "nanobanana",
    aliases: ["nano-banana", "nb", "bananaai"],
    version: "1.1.0",
    author: "frnAlt",
    countDown: 5,
    role: 0,
    shortDescription: { en: "Chat & generate images with Nano Banana AI" },
    longDescription: { en: "Interact with Nano Banana model for fast text generation, reasoning, and image synthesis." },
    category: "ai",
  ...
  onStart: async function ({ message, args, event, api, commandName }) {
    ...
    if (!prompt && !imageUrl) {
      return message.reply("🍌 Please provide a prompt or reply to an image for Nano Banana AI.");
    }

    return this.handleNanoBanana({ message, event, api, prompt, imageUrl, commandName });
  },
  ...
  handleNanoBanana: async function ({ message, event, api, prompt, imageUrl, commandName, history = [] }) {
    setReaction(api, message, "🍌", event.messageID);
    ...
    try {
      ...
      let responseText = `🍌 **[Nano Banana AI]**\n━━━━━━━━━━━━━━━━\n${result.content}`;
      ...
      if (result.image_urls && result.image_urls.length > 0) {
        ...
        for (let i = 0; i < result.image_urls.length; i++) {
          const imgPath = path.join(cacheDir, `nanobanana_${Date.now()}_${i}.png`);
      ...
      setReaction(api, message, "❌", event.messageID);
      message.reply(`❌ **Nano Banana Error:** ${error.message}`);
    }
  }
};
