...
function buildAPI(globalOptions, html, token, jar) {
  ...
  console.log(co("[ LOGIN ]"), (cra("[ FCA ]")), `Logged in as ${userID}`);
  ...
  if (token == 'NONE') 
    ...
    console.log(co("[ LOGIN ]"), (cra("[ FCA ]")), `Cant get access_token`);
  if (oldFBMQTTMatch) {
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function() {
      console.log(co("[ LOGIN ]"), (cra("[ FCA ]")), 'Done logging in.'); return callback(null, api);
    })
    .catch(function(e) {
