...
try {
    ...
    Module._resolveFilename = function(req, parent, isMain, opts) {
        if (req === 'uuid/v4') {
            try { return origResolve.call(this, 'uuid', parent, isMain, opts); } catch (_) {}
        }
...
if (global.Fca && global.Fca.Require && global.Fca.Require.Priyansh && global.Fca.Require.Priyansh.Config != 'default') {
    //do ssth
}
...
var utils = global.Fca.Require.utils,
    ...
    log = global.Fca.Require.log || { maxRecordSize: 100, pause: () => {}, resume: () => {}, info: console.log, warn: console.warn, error: console.error },
    { join } = require('path'),
    ...
    chalk = require("chalk"),
    figlet = (() => { try { return require("figlet"); } catch (_) { return { textSync: s => s }; } })(),
    os = require("os"),
    deasync = (() => { try { return require("deasync"); } catch (_) { return { loopWhile: () => {} }; } })(),
    Security = require("./Extra/Security/Base"),
    { getAll, deleteAll } = require('./Extra/ExtraGetThread'),
    ws = require('ws'),
    Websocket = require('./Extra/Src/Websocket'),
    Convert = (() => { try { return require('ansi-to-html'); } catch (_) { return class { toHtml(s) { return s; } }; } })();
    ...
    if ((maybeUser.length === 0 && maybeTiktik.length === 0) || !fb_dtsg) {
        ...
        var api = {
            ...
            getHealthStatus: function getHealthStatus() {
                const mqttConnected = !!(ctx.mqttClient && (ctx.mqttClient.connected !== false));
                return {
                    mqtt: { connected: mqttConnected },
                    antiSuspension: { circuitBreakerOpen: false },
                    uptime: process.uptime(),
                    timestamp: Date.now()
                };
            },
            stopListening: function stopListening(cb) {
                if (ctx.mqttClient) {
                    try {
            ...
            stopListeningAsync: function stopListeningAsync() {
                return new Promise(resolve => api.stopListening(resolve));
            },
            sessionGuard: function sessionGuard() {
                return true;
            },
            __ctx: ctx,
            ctx: ctx
