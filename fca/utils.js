...
try {
    ...
    log = { info: console.log, warn: console.warn, error: console.error, level: "info", pause: () => {}, resume: () => {} };
}
...
function sendRequestWithAntiBan(op, retries = 3) {
    let attempt = 0;
    function execute() {
        attempt++;
        return request(op).then(function(res) {
            let isBanned = false;
            try {
                const { isIpBannedOrRateLimited } = require("./src/utils/ipSpoofing");
                isBanned = isIpBannedOrRateLimited(res);
            } catch (_) {}
            if (isBanned && attempt < retries) {
                try {
                    const { globalIpBanProtection } = require("./src/utils/ipSpoofing");
            ...
            try {
                const { isIpBannedOrRateLimited } = require("./src/utils/ipSpoofing");
                isBanned = isIpBannedOrRateLimited(err.response || err.statusCode || 0, err);
            } catch (_) {}
            if (isBanned && attempt < retries) {
                try {
                    const { globalIpBanProtection } = require("./src/utils/ipSpoofing");
...
function get(url, jar, qs, options, ctx) {
    ...
    return sendRequestWithAntiBan(op);
}
