...
module.exports = function (defaultFuncs, api, ctx) {
  function uploadAttachment(attachments, callback) {
    ...
    for (var i = 0; i < attachments.length; i++) {
      var isStream = utils.isReadableStream(attachments[i]);
      var isBuf = Buffer.isBuffer(attachments[i]);
      if (!isStream && !isBuf) throw { error: "Attachment should be a readable stream or buffer and not " + utils.getType(attachments[i]) + "." };
